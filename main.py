...
def apply_ps4_header(squads_path):
    """
    Sostituisce l'header del file Squads PS4 con quello del file DATA reale.
    - Rimuove i primi 1178 byte (header generato da save_squads)
    - Prende l'header di DATA fino al marker T3DB (44 42 00 08)
    - Azzera i 4 byte che seguono "Type_Squads\x00" nell'header
    - Rinomina DATA in OLD_DATA e scrive il nuovo file DATA
    """
    if not os.path.isfile(DATA_PATH):
        print("DATA non trovato, skip header replacement")
        return

    print("Applico header PS4 da DATA...")

    with open(squads_path, "rb") as f:
        squads = f.read()

    # L'header generato da save_squads ha dimensione fissa: 1126 + 48 + 4 = 1178
    squads_header_size = 1126 + 48 + 4
    db_content = squads[squads_header_size:]

    if db_content[:2] != b"DB":
        print("  ERRORE: DB non trovato al byte {} di Squads".format(squads_header_size))
        return
    print("  DB confermato a offset {} (0x{:X}) in Squads".format(squads_header_size, squads_header_size))

    with open(DATA_PATH, "rb") as f:
        data = f.read()

    # Cerca il marker T3DB (44 42 00 08) nel DATA, ignorando eventuali "DB" nel nome salvataggio
    t3db_pos = data.find(b"\x44\x42\x00\x08")
    if t3db_pos < 1000:
        print("  ERRORE: T3DB non trovato dopo byte 1000 (trovato a {})".format(t3db_pos))
        return
    print("  T3DB trovato in DATA a byte {} (0x{:X})".format(t3db_pos, t3db_pos))

    # Copia l'header di DATA e azzera i 4 byte dopo "Type_Squads\x00"
    new_header = bytearray(data[:t3db_pos])
    type_squads_sig = b"Type_Squads\x00"
    ts_pos = new_header.find(type_squads_sig)
    if ts_pos == -1:
        print("  ERRORE: 'Type_Squads\\x00' non trovato nell'header di DATA")
        return
    crc_pos = ts_pos + len(type_squads_sig)
    print("  Type_Squads\\x00 a byte {}, CRC={} -> azzerato".format(ts_pos, new_header[crc_pos:crc_pos+4].hex()))
    new_header[crc_pos:crc_pos+4] = b"\x00" * 4
    new_data = bytes(new_header) + db_content

    if new_data[t3db_pos:t3db_pos+2] != b"DB":
        print("  ERRORE: DB non al posto atteso nel nuovo DATA")
        return
    print("  DB confermato a byte {} nel nuovo DATA".format(t3db_pos))

    old_data_path = os.path.join(os.path.dirname(os.path.abspath(DATA_PATH)), "OLD_DATA")
    ...
    if old_size == new_size:
        print("  Size check PASSATO: {} byte".format(new_size))
    else:
        print("  Size check ATTENZIONE: OLD_DATA={}, nuovo DATA={}".format(old_size, new_size))
...
if __name__ == "__main__":
    ...
    for platform in result["platforms"]:
        ...
        if not os.path.isdir(ver_path):
            ...
            squads_filename = "Squads{}000000".format(fdate)
            save_squads(buf, ver_path, squads_filename)

            if platform["name"] == "ps4":
                apply_ps4_header(os.path.join(ver_path, squads_filename))
